...
static SQRESULT Script_Player_IsSkydiveAnticipating(HSQUIRRELVM v)
{
	...
	// Freefall state 1 = anticipating (about to land)
	int state = *(int*)((uintptr_t)pPlayer + CPLAYER_M_FREEFALLSTATE_OFFSET);
...
//-----------------------------------------------------------------------------
// GetLastTimeDamaged - returns most recent damage time from any source
//-----------------------------------------------------------------------------
...
{
	int handle;
	int stanceType; // 0=STAND, 1=CROUCH (script values mapped to engine values)
};
...
{
	// Engine: 0=none, 1=stand, 2=crouch. Script: FORCE_STANCE_STAND=0, FORCE_STANCE_CROUCH=1
	int engineValue = 0;
	if (!ss.stack.empty())
	{
		int scriptStance = ss.stack.back().stanceType;
		engineValue = scriptStance + 1; // script 0->engine 1 (stand), script 1->engine 2 (crouch)
	}
...
static SQRESULT Script_SetShieldHealthFromSource(HSQUIRRELVM v)
{
	...
	// Call engine's SetShieldHealth via SetPlayerNetInt
	// Shield health is networked as "shieldHealth" player net int
	// The server script can set it directly
	SCRIPT_CHECK_AND_RETURN(v, SQ_OK);
}
...
static SQRESULT Script_IsMostRecentShieldChangeFromSingleSource(HSQUIRRELVM v)
{
	...
	// Walk history backwards looking for matching shield amount within time window
	...
	for (int n = 0; n < SHIELD_HISTORY_SIZE; n++)
	{
		...
		// Found matching entry - check if any OTHER source contributed
...
//-----------------------------------------------------------------------------
// Purpose: TrySelectOffhand - requests the engine to select an offhand weapon
//          Stub: engine field does not exist, logs the request
//-----------------------------------------------------------------------------
static SQRESULT Script_TrySelectOffhand(HSQUIRRELVM v)
{
	...
	Warning(eDLL_T::CLIENT, "TrySelectOffhand(%d) called - stub\n",
...
void Script_RegisterPlayerScriptFunctions(ScriptClassDescriptor_t* playerStruct)
{
	...
	// Skydive aliases (mapped to freefall)
	playerStruct->AddFunction(
	...
	// Skyward stubs (no engine support)
	playerStruct->AddFunction(
